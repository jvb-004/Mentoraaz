  ...
  return (
    <div className="auth-page">
      <div className="card auth-card">
        ...
        <h1>Hesab yarat</h1>
        <p className="auth-sub">Pulsuz, 30 saniyə çəkir</p>

        <div className="role-picker">
          <button type="button" className={`role-btn${isTutor === false ? ' selected' : ''}`} onClick={() => setIsTutor(false)}>
            <span>🎓</span><strong>Müəllim axtarıram</strong><small>Övladım / özüm üçün</small>
          </button>
          <button type="button" className={`role-btn${isTutor === true ? ' selected' : ''}`} onClick={() => setIsTutor(true)}>
            <span>👩‍🏫</span><strong>Mən müəllimim</strong><small>Tələbə tapmaq istəyirəm</small>
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p style={{fontSize:12,color:'var(--grey-400)',textAlign:'center',marginTop:12}}>
            Qeydiyyatdan keçməklə <Link to="/terms" style={{color:'var(--teal)'}}>İstifadə Şərtləri</Link> və <Link to="/privacy" style={{color:'var(--teal)'}}>Məxfilik Siyasəti</Link>ni qəbul edirsən.
          </p>
        </form>
